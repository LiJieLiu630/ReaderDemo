...
public class PageFactory {
    ...
    private List<BookMixAToc> chaptersList;
    ...
    public PageFactory(Context context, String bookId, List<BookMixAToc> chaptersList) {
    ...
    public synchronized void onDraw(Canvas canvas) {
        ...
        if (mLines.size() > 0) {
            ...
            // 绘制背景 不设置的话数据回覆盖（类似于清空画布）
            if (mBookPageBg != null) {
                Log.w(TAG, "onDraw: 434");
                canvas.drawBitmap(mBookPageBg, null, rectF, null);
            } else {
                Log.w(TAG, "onDraw: 444");
