...
public abstract class BaseReadView extends View {
    ...
    public BaseReadView(Context context, String bookId, List<BookMixAToc> chaptersList,
    ...
    public synchronized void init(int theme) {
        if (!isPrepared) {
            try {
                ...
                Log.i("55", "上次阅读位置：chapter=" + pos[0] + " startPos=" + pos[1] + " endPos=" + pos[2]);
                if (ret == 0) {
                ...
                if (actiondownX >= mScreenWidth / 3 && actiondownX <= mScreenWidth * 2 / 3
                    ...
                    if (actiondownX < mScreenWidth / 2) {// 从左翻
                        ...
                        if (status == BookStatus.NO_PRE_PAGE) {
                            Toast.makeText(MyReaderApplication.getContext(), "没有上一页啦", Toast.LENGTH_LONG).show();
                            return false;
                        ...
                        if (status == BookStatus.NO_NEXT_PAGE) {
                            Toast.makeText(MyReaderApplication.getContext(), "没有下一页啦", Toast.LENGTH_LONG).show();
                            return false;
                        } else if (status == BookStatus.LOAD_SUCCESS) {
                            abortAnimation();
                            pagefactory.onDraw(mNextPageCanvas);
